...
function CastRelocate( event )
	...
	if caster:HasModifier( tetherModifierName ) then
		...
		if not IsRelocatableUnit( ability.tetheredAlly ) then
			ability.tetheredAlly = nil
		end
	end
...
function Teleportation_PreDestroyTree( event )
	...
	if caster:HasModifier( tetherModifierName ) then
		...
		if ability.tetheredAlly and not (ability.tetheredAlly:HasModifier(tether_ally_modifier) and IsRelocatableUnit( ability.tetheredAlly )) then
			ability.tetheredAlly = nil
		end
	end
	ability.relocate_tetheredAlly = ability.tetheredAlly
	...
	if ability.tetheredAlly then
		pfx = ParticleManager:CreateParticle( event.teleport_particle, PATTACH_POINT, ability.tetheredAlly )
		ParticleManager:SetParticleControlEnt( pfx, 0, ability.tetheredAlly, PATTACH_POINT, "attach_hitloc", ability.tetheredAlly:GetAbsOrigin(), true )
	end
...
function Teleportation_PostDestroyTree( event )
	...
	ability.tetheredAlly = ability.relocate_tetheredAlly
	if ability.tetheredAlly then
		FindClearSpaceForUnit( ability.tetheredAlly, ability.relocate_targetPoint + Vector( 16, 16, 0 ), false )
		ability.tetheredAlly:Stop()
