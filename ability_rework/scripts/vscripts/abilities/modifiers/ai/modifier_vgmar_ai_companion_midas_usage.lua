...
function modifier_vgmar_ai_companion_midas_usage:OnIntervalThink()
	...
	local function MidasACreep( midas )
		local target = nil
		local enemycreeps = {}
		enemycreeps = FindUnitsInRadius(3, parent:GetAbsOrigin(), nil, 800, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_CLOSEST, false)
		if #enemycreeps > 0 then
			...
			for i=1,#enemycreeps do
				...
				if enemycreeps[i] ~= nil and ((enemycreeps[i]:GetAbsOrigin() - parent:GetAbsOrigin()):Length2D() < 800 and parent:CanEntityBeSeenByMyTeam(enemycreeps[i])) and not (enemycreeps[i]:IsTower() or enemycreeps[i]:IsBuilding() or enemycreeps[i]:IsSummoned() or enemycreeps[i]:IsCourier()) then
		...
		local neutralcreeps = {}
		neutralcreeps = FindUnitsInRadius(4, parent:GetAbsOrigin(), nil, 800, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_CREEP, DOTA_UNIT_TARGET_FLAG_NOT_ANCIENTS + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_CLOSEST, false)
		if #neutralcreeps > 0 then
			...
			for i=1,#neutralcreeps do
				...
				if neutralcreeps[i] ~= nil and ((neutralcreeps[i]:GetAbsOrigin() - parent:GetAbsOrigin()):Length2D() < 800 and parent:CanEntityBeSeenByMyTeam(neutralcreeps[i])) then
					if parent:GetLevel() == 25 then
		...
		if target ~= nil and shouldmove() and ((target:GetAbsOrigin() - parent:GetAbsOrigin()):Length2D() < 800 and parent:CanEntityBeSeenByMyTeam(target)) then
